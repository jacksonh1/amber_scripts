...
def strip_trajectories(remd_folder: Path, export_folder_name: str = "300K_export", temperature: int = 300, overwrite: bool = False):
    search_folder = remd_folder / f"analysis_demux_{temperature}K"
    if not search_folder.exists():
        raise FileNotFoundError(f"couldn't find {search_folder}")
    print(f"Processing folder: {search_folder}")
    output_dir = remd_folder / export_folder_name
...
def main(remd_folder: Path, export_folder_name: str = "300K_export", temperature: int = 300, overwrite: bool = False):
    strip_trajectories(remd_folder, export_folder_name, temperature, overwrite=overwrite)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--remd-folder", default="./output_T-REMD/", help="Path to the REMD output folder, should contain an 'analysis_demux_300K' folder")
    ...
    print(f"Using MD folder: {Path(args.remd_folder).resolve()}")
    main(
        remd_folder=Path(args.remd_folder),
